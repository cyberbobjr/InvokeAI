...
class OpenAIAnalyzeImageInvocation(BaseInvocation):
    ...
    prompt: str = InputField(
        default="Analyze this image and identify any recognizable characters, brands, or copyrighted elements if possible. Return a detailed description which will be used to generate an image. So don't start with 'This is an image of' or similar phrases.",
        description="The prompt for GPT-4 Vision to analyze the image"
    )
    ...
    def invoke(self, context: InvocationContext) -> StringOutput:
        ...
        if self.model_architecture == "tag_based":
            analysis_prompt = f"{self.prompt} FORMAT YOUR RESPONSE AS COMMA-SEPARATED TAGS ONLY (like: tag1, tag2, tag3) IN ENGLISH. No sentences, no descriptions, only tags suitable for SD1.5."
        else:  # sentence_based
            analysis_prompt = f"{self.prompt} FORMAT YOUR RESPONSE IN ENGLISH SENTENCES. Provide a detailed description in proper English." 
